...
PREPARED_HEIGHT = 720
PREPARED_FPS = 30
SAM3_TRACKING_WIDTH = 640
SAM3_TRACKING_HEIGHT = 360
SAM3_TRACKING_FPS = 30
...
def probe_sam3_tracking_video(video_path: Path) -> PreparedVideoProbe:
    """Probe and validate a SAM3-only tracking video using PyAV."""

    probe = _probe_video(video_path)
    validate_sam3_tracking_video_probe(probe)
    return probe
...
def validate_sam3_tracking_video_probe(probe: PreparedVideoProbe) -> None:
    if probe.width != SAM3_TRACKING_WIDTH:
        raise ValueError(
            "SAM3 tracking video width must be "
            f"{SAM3_TRACKING_WIDTH}, got {probe.width}"
        )
    if probe.height != SAM3_TRACKING_HEIGHT:
        raise ValueError(
            "SAM3 tracking video height must be "
            f"{SAM3_TRACKING_HEIGHT}, got {probe.height}"
        )
    if abs(probe.fps - SAM3_TRACKING_FPS) > 0.001:
        raise ValueError(
            f"SAM3 tracking video fps must be {SAM3_TRACKING_FPS}, got {probe.fps:g}"
        )
    if probe.frame_count <= 0:
        raise ValueError("SAM3 tracking video must contain at least one frame")
    if probe.has_audio:
        raise ValueError("SAM3 tracking video must not contain audio streams")


def _probe_video(video_path: Path) -> PreparedVideoProbe:
    if not video_path.exists():
        raise FileNotFoundError(f"Video file not found: {video_path}")
    if not video_path.is_file():
        raise ValueError(f"Video path is not a file: {video_path}")
