    ...
    def process(self, request: Sam3ExtractRequest) -> Sam3ExtractResponse:
        ...
        for seed_idx, seed in enumerate(request.seeds):
                ...
                if i == start_idx:
                    if seed.prompt:
                        inputs = self.processor(
                            images=image, text=seed.prompt, return_tensors="pt"
                        )
                    elif seed.points:
                        input_points = [[[p.x, p.y] for p in seed.points]]
                        input_labels = [
                            [[1 if p.is_positive else 0 for p in seed.points]]
                        ]
                        if seed.bbox_xyxy:
                            inputs = self.processor(
                                images=image,
                ...
                for box, score in zip(boxes, scores):
                    ...
                    if i == start_idx:
                        ...
                        if (
                            ...
                            and combined_score > request.match_threshold
                        ):
            ...
            if len(points_out) >= request.min_points:
                tracks.append(
                    EntityTrack(track_id=track_id, points=points_out, confidence=1.0)
                )
